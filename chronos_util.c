...
void mark_deadlocks(struct list_head *head, int i)
{
...
EXPORT_SYMBOL(timespec_to_long);
...
inline long update_left(struct rt_info *task)
{
	long left = 0;
	left = calc_left(task);
...
{
	struct rt_info *task, *a, *b;
	list_for_each_entry(task, head, task_list[LOCAL_LIST])
        {
		a=local_task(head->next);
		b=local_task(head->next->next);
		while(b != task && compare_before(a, b, SORT_KEY_LVD))
		{
			list_move_after(a, b, LOCAL_LIST);
			a=b;
			b=task_list_entry(b->task_list[LOCAL_LIST].next, LOCAL_LIST);
		}
	}
}

static void insert_list(struct rt_info *item, struct list_head *head, int i, int key, int before)
{
        struct rt_info *task;
	if(list_empty(head))
	{
		list_add_tail(&item->task_list[i], head);
		return;
	}	

        list_for_each_entry(task, head, task_list[i])
	{
                if(compare(item, task, key, before)) {
                        list_add_before(task, item, i);
                        return;
                }
        } 

      // if(task_list_entry(item->task_list[i].next, i) == item)
       list_add_tail(&item->task_list[i], head);
//                list_add_before(local_task(head),item, i);
}

static int list_feasible(struct list_head *head, int i)
{
        struct rt_info *task;
      ...
      struct timespec exec_ts = CURRENT_TIME;
        list_for_each_entry(task, head, task_list[i])
	{
                add_ts(&exec_ts, &(task->left), &exec_ts);
                if(earlier_deadline(&(task->deadline), &exec_ts))
                        return 0;
        } 
...
struct rt_info* dasa_task(struct list_head *head,int flags)
{
	struct rt_info *task;
	...
	list_for_each_entry(task, head, task_list[LOCAL_LIST])
	{
		if(check_task_failure(task,flags))							return task;

		livd(task,false,flags);
//	printk(KERN_EMERG"ivd %ld \n",task->local_ivd);
	}
	...
	{
//		printk(KERN_EMERG"ivd %ld \n",task->local_ivd);
		insert_list(task,&head1,SCHED_LIST1,SORT_KEY_DEADLINE,1);
		if(!(list_feasible(&head1,SCHED_LIST1)))
			list_remove(task,SCHED_LIST1);
	}
	...
	{
		return list_entry(head1.next,struct rt_info, task_list[SCHED_LIST1]);
	}


}
EXPORT_SYMBOL(dasa_task);
static int task_in_list( struct rt_info *task, struct list_head *head, int i)
{

	struct rt_info *temp;
	list_for_each_entry(temp, head,task_list[i])
        {
       		 if(temp==task)
                 {
                	 return 1;
...
struct rt_info* dasa_call(struct list_head *head, int flags)
{
        struct rt_info *task, *dependency;
        struct list_head head1;
        INIT_LIST_HEAD(&head1);

        list_for_each_entry(task, head, task_list[LOCAL_LIST])
        {
       		 printk(KERN_EMERG"OUTSIDE");       
	 	if(check_task_failure(task, flags))
			 return task;
printk(KERN_EMERG"BEFORE INITIALIZE DEP");
		initialize_dep(task);
              //  livd(task, true, flags);
//      printk(KERN_EMERG"ivd %ld \n",task->local_ivd);
        }
printk(KERN_EMERG"BEFORE MARKDEAD");

	mark_deadlocks(head, 0);

	list_for_each_entry(task, head, task_list[LOCAL_LIST])
        {
		printk(KERN_EMERG"BEFORE livd");
	...
	list_for_each_entry(task,head,task_list[LOCAL_LIST])
       {
		if(!task_in_list(task, &head1, SCHED_LIST1)){	
		
			insert_list(task, &head1, SCHED_LIST1, SORT_KEY_DEADLINE, 1);
			dependency=task->dep;
			while(task->dep!=NULL)
			{
				if(task_in_list(dependency, &head1, SCHED_LIST1))
				{
 					if(earlier_deadline(&task->deadline, &dependency->deadline))	//task has lesser deadline than temp
					{
						list_remove(dependency,SCHED_LIST1);
						dependency->deadline = task->deadline;
					}
					insert_list(dependency, &head1, SCHED_LIST1, SORT_KEY_DEADLINE, 1);
...
EXPORT_SYMBOL(dasa_call);

struct rt_info* lbesa_task(struct list_head *head,int flags)
{

        struct rt_info *task,*remove_task;
	...
	INIT_LIST_HEAD(&head2);
        list_for_each_entry(task,head,task_list[LOCAL_LIST])
        {
		 ...
		 if(check_task_failure(task,flags))                                      return task;
		livd(task,false,flags);	
		list_add_tail(&task->task_list[SCHED_LIST1],&head1);
//printk(KERN_EMERG"ivd %ld \n",task->local_ivd);
//		list_add_tail(&task->task_list[SCHED_LIST2],&head2);
	insert_list(task,&head2,SCHED_LIST2,SORT_KEY_LVD,1);

        }
//	insert_sort(&head2);
  
      while(!(list_feasible(&head1,SCHED_LIST1)))
       		 {
			if(list_empty(&head1))
                		return local_task(head->next);
			remove_task=list_last_entry(&head2,struct rt_info,task_list[SCHED_LIST2]);
			list_remove(remove_task,SCHED_LIST1);
			list_remove(remove_task,SCHED_LIST2);
		}
        ...
        {
                //quicksort(dhead,SCHED_LIST1,SORT_KEY_DEADLINE,1);
                return list_entry(head1.next,struct rt_info, task_list[SCHED_LIST1]);
        }


}
EXPORT_SYMBOL(lbesa_task);
